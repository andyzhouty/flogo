...
func addComment(accessToken string, content string, postId int, replyingId int) (comment Comment, err error) {
	...
	client := &http.Client{}
	req, err := http.NewRequest("POST", URLFor("/api/v3/comment/add"), bytes.NewReader(body))
	cobra.CheckErr(err)
